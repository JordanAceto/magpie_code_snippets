 ...
 * SD cards. This module would accomodate up to 8 SD cards if they were installed on the PCB. 6 cards was chosed because
...
#define MAX7312_7_BIT_I2C_ADDR (0x40u)
...
#define MAX7312_MUX_CTL_PINS_IN_PORT0_MASK ( \
...
#define MAX7312_SD_EN_PINS_IN_PORT0_MASK ( \
...
#define MAX7312_SD_EN_PINS_IN_PORT1_MASK ( \
...
#define MAX7312_SD_DETECT_PINS_IN_PORT1_MASK ( \
 ...
 */
static SD_Card_Bank_Error_t max7312_reg_write(MAX7312_Register_Addr_t reg, uint8_t val);
 ...
 */
static SD_Card_Bank_Error_t max7312_reg_read(MAX7312_Register_Addr_t reg, uint8_t *read_byte);

/* Public function definitions ---------------------------------------------------------------------------------------*/

SD_Card_Bank_Error_t sd_card_bank_init(I2C_HandleTypeDef *hi2c)
{
    hi2c_ = hi2c;
    // Reset all ports to zero, they initialize to 1 on power up.
    if (sd_card_bank_disable_all() != SD_CARD_BANK_ERROR_ALL_OK)
    {
        return SD_CARD_BANK_I2C_ERROR;
    }
    ...
    if (max7312_reg_write(MAX7312_REG_CONFIG_0, ~port_0_output_pins) != SD_CARD_BANK_ERROR_ALL_OK)
    {
        return SD_CARD_BANK_I2C_ERROR;
    }
    ...
    if (max7312_reg_write(MAX7312_REG_CONFIG_1, ~port_1_output_pins) != SD_CARD_BANK_ERROR_ALL_OK)
    {
        return SD_CARD_BANK_I2C_ERROR;
    }
...
SD_Card_Bank_Error_t sd_card_bank_disable_all()
{
    active_card_slot = SD_CARD_BANK_ALL_SLOTS_DISABLED;

    if (max7312_reg_write(MAX7312_REG_OUTPUT_PORT_0, 0x00u) != SD_CARD_BANK_ERROR_ALL_OK)
    {
        return SD_CARD_BANK_I2C_ERROR;
    }

    if (max7312_reg_write(MAX7312_REG_OUTPUT_PORT_1, 0x00u) != SD_CARD_BANK_ERROR_ALL_OK)
    {
        return SD_CARD_BANK_I2C_ERROR;
    }

    return SD_CARD_BANK_ERROR_ALL_OK;
}

SD_Card_Bank_Error_t sd_card_bank_enable_slot(SD_Card_Bank_Card_Slot_t slot)
{
    // early return on bad inputs
    if (slot >= SD_CARD_BANK_NUM_CARDS)
    {
        return SD_CARD_BANK_INVALID_INPUT_ERROR;
    }
    ...
    if (max7312_reg_write(MAX7312_REG_OUTPUT_PORT_0, port_0_data) != SD_CARD_BANK_ERROR_ALL_OK)
    {
        return SD_CARD_BANK_I2C_ERROR;
    }

    if (max7312_reg_write(MAX7312_REG_OUTPUT_PORT_1, port_1_data) != SD_CARD_BANK_ERROR_ALL_OK)
    {
        return SD_CARD_BANK_I2C_ERROR;
    }
...
SD_Card_Bank_Card_Slot_t sd_card_bank_get_active_slot()
{
    return active_card_slot;
}

SD_Card_Bank_Error_t sd_card_bank_read_and_cache_detect_pins()
{
    uint8_t read_byte;

    if (max7312_reg_read(MAX7312_REG_INPUT_PORT_1, &read_byte) != SD_CARD_BANK_ERROR_ALL_OK)
    {
        return SD_CARD_BANK_I2C_ERROR;
    }
...
bool sd_card_bank_active_card_is_inserted()
{
...
SD_Card_Bank_Error_t max7312_reg_write(MAX7312_Register_Addr_t reg, uint8_t val)
{
    ...
    HAL_StatusTypeDef res = HAL_I2C_Master_Transmit(hi2c_, MAX7312_7_BIT_I2C_ADDR, max_7312_i2c_buff, num_bytes_to_write, HAL_MAX_DELAY);

    return res == HAL_OK ? SD_CARD_BANK_ERROR_ALL_OK : SD_CARD_BANK_I2C_ERROR;
}

SD_Card_Bank_Error_t max7312_reg_read(MAX7312_Register_Addr_t reg, uint8_t *read_byte)
{
    // first we need to write the address of the register we want to read
    ...
    // then we can read a single byte from that register
    const uint16_t num_bytes_to_read = 1u;
    res = HAL_I2C_Master_Receive(hi2c_, MAX7312_7_BIT_I2C_ADDR, max_7312_i2c_buff, num_bytes_to_read, HAL_MAX_DELAY);
