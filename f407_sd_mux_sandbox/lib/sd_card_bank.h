 ...
 * querying whether an SD card is inserted or not.
 */
#ifndef SD_CARD_BANK_H_
#define SD_CARD_BANK_H_
...
#define SD_CARD_BANK_NUM_CARDS (6u)
...
typedef enum
{
    SD_CARD_BANK_ERROR_ALL_OK,
    SD_CARD_BANK_I2C_ERROR,
    SD_CARD_BANK_INVALID_INPUT_ERROR
} SD_Card_Bank_Error_t;

/* Public function declarations --------------------------------------------------------------------------------------*/

/**
 * @brief `sd_card_bank_init(i2c)` initializes the SD card bank with I2C handle `i2c` and configures all of the I2C
...
/**
 * @brief `sd_card_bank_disable_all()` deselects any active card slot and powers down the SD card bank.
 *
...
/**
 * @brief `sd_card_bank_enable_slot(s)` powers on the SD card bank and selects card slot `s` as the active card.
 *
...
/**
 * @brief `sd_card_bank_get_active_slot()` is the current enumerated active card slot.
 *
 * @pre `sd_card_bank_init()` has been called.
 *
...
/**
 * @brief `sd_card_bank_read_and_cache_detect_pins()` reads the SD card detect pins and caches the result.
 *
 * @pre `sd_card_bank_init()` has been called.
 *
...
/**
 * @brief `sd_card_bank_active_card_is_inserted()` is true iff a card in the active slot is physically detected.
 *
